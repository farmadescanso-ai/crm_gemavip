...
router.get('/new', requireLogin, async (_req, res, next) => {
  try {
    const [comerciales, tarifas, provincias, paises, formasPago, tiposClientes, especialidades, idiomas, monedas, estadosCliente, cooperativas, gruposCompras, meta] = await Promise.all([
      ...
      loadSimpleCatalogForSelect(db, 'tipos_clientes', { labelCandidates: ['tipc_tipo', 'tipc_nombre', 'Tipo', 'Nombre'] }).then((r) => (Array.isArray(r) && r.length > 0 ? r : db.getTiposClientes?.().catch(() => []))),
      Promise.resolve(db.getEspecialidades?.().catch(() => [])).then((r) => (Array.isArray(r) && r.length > 0 ? r : loadSimpleCatalogForSelect(db, 'especialidades', { labelCandidates: ['esp_nombre', 'Nombre', 'nombre', 'Especialidad'] }))),
