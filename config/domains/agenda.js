...
module.exports = {
  ...
  async getContactoById(id) {
    try {
      ...
      if (item) {
        const colNombre = pick(['ag_nombre', 'Nombre', 'nombre']);
        const colApellidos = pick(['ag_apellidos', 'Apellidos', 'apellidos']);
        const colEmpresa = pick(['ag_empresa', 'Empresa', 'empresa']);
        const colEmail = pick(['ag_email', 'Email', 'email']);
        const colMovil = pick(['ag_movil', 'Movil', 'movil']);
        const colTelefono = pick(['ag_telefono', 'Telefono', 'telefono']);
        const colExtension = pick(['ag_extension', 'Extension', 'extension']);
        const colNotas = pick(['ag_notas', 'Notas', 'notas']);
        const colActivo = pick(['ag_activo', 'Activo', 'activo']);
        item.Id = item[colPk] ?? item.ag_id ?? item.Id ?? item.id ?? null;
        item.id = item.Id;
        item.Nombre = item[colNombre] ?? item.Nombre ?? item.nombre ?? '';
        item.Apellidos = item[colApellidos] ?? item.Apellidos ?? item.apellidos ?? '';
        item.Empresa = item[colEmpresa] ?? item.Empresa ?? item.empresa ?? '';
        item.Email = item[colEmail] ?? item.Email ?? item.email ?? '';
        item.Movil = item[colMovil] ?? item.Movil ?? item.movil ?? '';
        item.Telefono = item[colTelefono] ?? item.Telefono ?? item.telefono ?? '';
        item.Extension = item[colExtension] ?? item.Extension ?? item.extension ?? '';
        item.Notas = item[colNotas] ?? item.Notas ?? item.notas ?? '';
        item.Activo = item[colActivo] ?? item.Activo ?? item.activo ?? 1;
        item.Id_TipoCargoRol = item[colTipoCargoRol] ?? item.ag_tipcar_id ?? item.Id_TipoCargoRol ?? item.id_tipocargorol ?? null;
        item.Id_Especialidad = item[colEspId] ?? item.ag_esp_id ?? item.Id_Especialidad ?? item.id_especialidad ?? null;
