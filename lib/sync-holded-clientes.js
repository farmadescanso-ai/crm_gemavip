...
/**
 * Contacto Holded considerado «cliente» (excluye proveedor si la API lo indica).
 * @param {object} contact
 */
function isHoldedContactCliente(contact) {
  if (!contact || typeof contact !== 'object') return true;
  const t = String(contact.type ?? contact.contactType ?? contact.role ?? '').toLowerCase();
  if (t === 'supplier' || t === 'proveedor' || t === 'suppliers') return false;
  if (t === 'client' || t === 'customer' || t === 'cliente' || t === 'clients') return true;
  if (contact.isSupplier === true || contact.supplier === true) return false;
  if (contact.isClient === true || contact.client === true) return true;
  return true;
}
...
async function buildHoldedPreviewRowsFromList(db, list, selectedLower, opt = {}) {
  ...
  let holdedContactosClientes = 0;
  for (const cx of list) {
    if (isHoldedContactCliente(cx)) holdedContactosClientes++;
  }
