...
function buildClientePayloadFromHoldedContact(contact, ctx) {
  ...
  const holdedId = String(contact.id ?? contact._id ?? '').trim();
  ...
  if (notes != null && String(notes).trim() !== '') payload.Observaciones = String(notes).trim();
...
async function importHoldedClientesEs(db, opts = {}) {
  ...
  for (const row of toImport) {
    ...
    try {
      ...
      if (!full || (typeof full === 'object' && !full.id && !full._id)) {
        errors++;
        continue;
      }
      ...
      if (!existing?.length && cif) {
        existing = await db.query('SELECT cli_id FROM clientes WHERE cli_dni_cif = ? LIMIT 1', [cif]);
      }
