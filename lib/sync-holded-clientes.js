...
async function ensureColumnCliIdHolded(db) {
  try {
    ...
    const cols = await db.query(
      `SELECT COLUMN_NAME, CHARACTER_MAXIMUM_LENGTH AS maxlen, DATA_TYPE AS dt
       FROM information_schema.COLUMNS
       WHERE TABLE_SCHEMA = DATABASE()
         AND LOWER(TABLE_NAME) = 'clientes'
         AND COLUMN_NAME = 'cli_Id_Holded'`
    );
    if (!cols?.length) {
      await db.query(
    ...
    const dt = String(cols[0].dt || '').toLowerCase();
    const maxlen = cols[0].maxlen != null ? Number(cols[0].maxlen) : null;
    const tooShort =
      (dt === 'varchar' || dt === 'char') && maxlen != null && Number.isFinite(maxlen) && maxlen < 255;
    if (tooShort) {
      await db.query(
        'ALTER TABLE `clientes` MODIFY COLUMN `cli_Id_Holded` VARCHAR(255) NULL DEFAULT NULL'
      );
    }
  } catch (e) {
...
async function enrichHoldedRowsWithSyncState(db, rows, list, paisIdEspana) {
  ...
  for (const row of candidatos) {
    ...
    if (H === C) {
      ...
      row.motivo = 'Cambios en CRM respecto a Holded';
    } else {
