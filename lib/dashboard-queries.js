...
async function queryKpiVentasYPedidos(db, {
  ...
  let ventasParams = [];
  if (pedidosWithZone) {
    ventasSql += ` INNER JOIN \`${tClientes}\` c ON c.\`${pkClientes}\` = p.\`${colPedCliente}\` ${ccaaJoin} ${extraJoinsWithZone} ${pedWhereClause} ${zoneCondition}`;
    ventasParams = [...pedWhereParams, ...zoneParams];
  } else {
    ventasSql += ` ${extraJoinsNoZone} ${pedWhereClause}`;
...
async function queryDesgloseEstadoPedidos(db, pedidosCols, {
  ...
  const epJoin = !String(pedidoExtraJoins || '').trim()
  ...
  const deSql = `
    ...
    ${String(pedidoExtraJoins || '').trim() ? pedidoExtraJoins : epJoin}
    ${deWhereClause}
...
async function queryLatestPedidosAdminDashboard(db, {
  ...
  return db.query(
    `SELECT p.ped_id AS Id, p.\`${colPedNum}\` AS NumPedido, p.\`${colPedFecha}\` AS FechaPedido, p.\`${colPedTotal}\` AS TotalPedido, p.\`${colPedEstado}\` AS EstadoPedido
     FROM \`${tPedidos}\` p ${extraJoins} ${pedWhereClause} ORDER BY p.ped_id DESC LIMIT ${limitAdmin}`,
    pedWhereParams
  );
}
...
async function queryMisClientesComercialDashboard(db, metaVisitas, clientesMeta, {
  ...
  return db.query(
    `SELECT c.\`${pkClientes}\` AS Id, c.\`${colNombreRazon}\` AS Nombre_Razon_Social,
     ...
     FROM \`${tClientes}\` c
     INNER JOIN \`${tPedidos}\` p ON p.\`${colPedCliente}\` = c.\`${pkClientes}\` AND p.\`${colPedComercial}\` = ? ${hasDateFilter ? `AND p.\`${colPedFecha}\` >= ? AND p.\`${colPedFecha}\` < ? + INTERVAL 1 DAY` : ''}
