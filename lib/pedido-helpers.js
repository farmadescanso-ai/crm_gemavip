...
async function buildStandardPedidoXlsxBuffer({ item, id, lineas, cliente, direccionEnvio, fmtDateES, mayoristaInfo }) {
  ...
  for (let ri = 1; ri <= 5; ri++) ws.getRow(ri).height = 18;
  ...
  for (let ri = 7; ri <= 12; ri++) ws.getRow(ri).height = 18;
...
async function buildHefameXlsxBuffer({ item, id, lineas, cliente, mayoristaInfo }) {
  ...
  const nombreClienteSafe = String(nombreClienteRaw)
    ...
    .replace(/\s/g, '_')
    .slice(0, 80) || 'cliente';
  const numAsociado = String(mayoristaInfo?.codigoAsociado || codigoHefame || '').trim().replace(/[\\/:*?"<>|]/g, '') || 'sin_asociado';
  const attachmentFileName = `${yyyymmdd}_${nombreClienteSafe}_${numAsociado}.xls`;
