...
async function resolveMayoristaInfo(db, item) {
  ...
  if (!codigoAsociado) {
    ...
    const coop = (cooperativas || []).find((c) => String(c.Nombre || c.nombre || '').toUpperCase() === mayoristaNombre.toUpperCase());
    ...
    if (!codigoAsociado) {
      ...
      const match = (cooperativasCliente || []).find((c) => String(c.Nombre || c.nombre || '').toUpperCase() === mayoristaNombre.toUpperCase());
