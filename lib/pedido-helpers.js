...
async function buildStandardPedidoXlsxBuffer({ item, id, lineas, cliente, direccionEnvio, fmtDateES }) {
  ...
  (Array.isArray(lineas) ? lineas : []).forEach((l) => {
    const codigo = rejectFilePath(
      _n(l.art_sku, _n(l.art_codigo_interno, _n(l.SKU, _n(l.Codigo, _n(l.Id_Articulo, l.id_articulo)))))
    );
    const concepto = rejectFilePath(
      _n(l.art_nombre, _n(l.art_descripcion, _n(l.Nombre, _n(l.Descripcion, _n(l.Articulo, l.nombre)))))
    );
