...
async function resolveMayoristaInfo(db, item) {
  ...
  if (!codigoAsociado) {
    ...
    const cooperativaId = coop ? Number(_n(coop.id, coop.Id)) : null;
...
async function buildHefameXlsxBuffer({ item, id, lineas, cliente, mayoristaInfo }) {
  ...
  try {
    ...
    if (mayoristaInfo && (mayoristaInfo.nombre || mayoristaInfo.codigoAsociado)) {
      if (mayoristaInfo.nombre) wsCabecera.getCell('C12').value = `Cooperativa: ${mayoristaInfo.nombre}`;
      if (mayoristaInfo.codigoAsociado) wsCabecera.getCell('C14').value = mayoristaInfo.codigoAsociado;
    }
  } catch (e) {
