...
async function buildStandardPedidoXlsxBuffer({ item, id, lineas, cliente, direccionEnvio, fmtDateES, mayoristaInfo }) {
  ...
  (Array.isArray(lineas) ? lineas : []).forEach((l) => {
    ...
    const baseCalc = Number.isFinite(baseStored)
      ? round2(baseStored)
      : round2(qty * pvl * (1 - dto / 100) * (1 - dtoPedidoPct / 100));
    ...
    r.getCell(6).value = baseCalc;
    r.getCell(7).value = ivaPct;
    r.getCell(8).value = totalCalc;
  ...
  ws.getRow(totalsStart).height = 5;
