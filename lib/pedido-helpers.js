...
const { toNum: toNumUtil, escapeHtml: escapeHtmlUtil, round2 } = require('./utils');
const { getTaxLabel } = require('./tax-helpers');
...
async function canShowHefameForPedido(db, item) {
  ...
  const tipo = _n((tipos || []).find((t) => Number(_n(t.tipp_id, _n(t.id, t.Id))) === idTipoPedido), null);
  ...
  const tipoNombre = String((tipo && (tipo.tipp_tipo || tipo.Tipo || tipo.Nombre || tipo.nombre)) || '').trim();
...
async function isTransferPedido(db, item) {
  ...
  const tipo = _n((tipos || []).find((t) => Number(_n(t.tipp_id, _n(t.id, t.Id))) === idTipoPedido), null);
  const tipoNombre = String((tipo && (tipo.tipp_tipo || tipo.Tipo || tipo.Nombre || tipo.nombre)) || '').trim();
  return /transfer/i.test(formaPagoNombre) || /transfer/i.test(tipoNombre);
}

/** Resuelve mayoristaInfo (cooperativa + nº asociado) usando Id_Cooperativa de clientes_cooperativas */
async function resolveMayoristaInfo(db, item) {
  ...
  const codigoPedido = String(_n(_n(_n(item && item.NumAsociadoHefame, item && item.num_asociado_hefame), _n(item && item.numero_cooperativa, item && item.NumeroCooperativa)), '')).trim();
  let codigoAsociado = codigoPedido;
  if (!codigoAsociado) {
    const cooperativas = await db.getCooperativas().catch(() => []);
    const nom = mayoristaNombre.toUpperCase();
    const coop = (cooperativas || []).find((c) => String(c.Nombre || c.nombre || '').toUpperCase() === nom) ||
      (cooperativas || []).find((c) => /hefame/i.test(String(c.Nombre || c.nombre || '')));
    const cooperativaId = coop ? Number(_n(_n(coop.id, coop.Id), coop.coop_id)) : null;
    if (cooperativaId && db.getNumAsociadoByClienteAndCooperativaId) {
      codigoAsociado = await db.getNumAsociadoByClienteAndCooperativaId(idCliente, cooperativaId);
    }
    if (!codigoAsociado) {
      const cooperativasCliente = await (db.getCooperativasByClienteId && db.getCooperativasByClienteId(idCliente).catch(() => [])) || [];
      const match = (cooperativasCliente || []).find((c) => String(c.Nombre || c.nombre || '').toUpperCase() === nom) ||
        (cooperativasCliente || []).find((c) => /hefame/i.test(String(c.Nombre || c.nombre || '')));
      codigoAsociado = match ? String(_n(match.NumAsociado, match.numAsociado)).trim() : '';
    }
    if (!codigoAsociado && db.getNumAsociadoHefameByClienteId) {
      codigoAsociado = await db.getNumAsociadoHefameByClienteId(idCliente).catch(() => null) || '';
    }
  }
  return { nombre: mayoristaNombre, codigoAsociado: codigoAsociado || null };
}
...
function pickCliente(obj, cands) {
  if (!obj || typeof obj !== 'object') return '';
  const map = new Map(Object.keys(obj).map((k) => [String(k).toLowerCase(), obj[k]]));
  for (const c of cands || []) {
    const v = map.get(String(c).toLowerCase());
    if (v != null && String(v).trim() !== '') return String(v).trim();
  }
  return '';
}

async function buildStandardPedidoXlsxBuffer({ item, id, lineas, cliente, direccionEnvio, fmtDateES, mayoristaInfo }) {
  const dtoPedidoPct = Math.max(0, Math.min(100, toNumUtil(_n(_n(_n(item.ped_dto, item.Dto), item.Descuento), 0), 0)));
  ...
  if (mayoristaInfo && (mayoristaInfo.nombre || mayoristaInfo.codigoAsociado)) {
    datosPedido += (mayoristaInfo.nombre ? `Cooperativa: ${mayoristaInfo.nombre}\n` : '') +
      (mayoristaInfo.codigoAsociado ? `Nº asociado: ${mayoristaInfo.codigoAsociado}\n` : '');
  }
  cRight.value = datosPedido;
  ...
  for (let ri = 1; ri <= 5; ri++) ws.getRow(ri).height = 21;
  ws.getRow(6).height = 13;

  ws.mergeCells('A7:D12');
  ws.mergeCells('E7:H12');
  const clienteId = pickCliente(cliente, ['cli_id', 'Id', 'id']) || (item && (item.Id_Cliente ?? item.ped_cli_id)) || '';
  const clienteNombre = pickCliente(cliente, ['cli_nombre_razon_social', 'Nombre_Razon_Social', 'Nombre', 'nombre', 'cli_nombre_cial', 'Nombre_Cial']);
  const clienteCif = pickCliente(cliente, ['cli_dni_cif', 'DNI_CIF', 'DniCif', 'dni_cif']);
  const clienteDir = pickCliente(cliente, ['cli_direccion', 'Direccion', 'direccion']);
  const clientePob = pickCliente(cliente, ['cli_poblacion', 'Poblacion', 'poblacion']);
  const clienteCp = pickCliente(cliente, ['cli_codigo_postal', 'CodigoPostal', 'codigo_postal']);
  const clienteEmail = pickCliente(cliente, ['cli_email', 'Email', 'email']);
  const clienteTel = pickCliente(cliente, ['cli_telefono', 'cli_movil', 'Telefono', 'Movil', 'telefono', 'movil']);

  const clienteLineas = [
    clienteId ? `ID: ${clienteId}` : '',
    clienteNombre || (item && item.Id_Cliente) || '',
    clienteCif,
    clienteDir,
    [clienteCp, clientePob].filter(Boolean).join(' ') || '',
    [clienteEmail, clienteTel].filter(Boolean).join(' · ') || ''
  ].filter(Boolean);
  ...
  for (let ri = 7; ri <= 12; ri++) ws.getRow(ri).height = 21;
  ...
  const _taxLabel = getTaxLabel(Number(item?.ped_regfis_id ?? cliente?.cli_regfis_id ?? 1) || 1);
  header.values = ['CÓDIGO', 'CONCEPTO', 'PVL', 'UNDS', 'DTO', 'SUBTOTAL', _taxLabel, 'TOTAL'];
  header.height = 21;
  ...
  const rejectFilePath = (v) => {
    const s = String(v || '').trim();
    if (/^[A-Za-z]:[\\/]|^\/[a-z]/.test(s) || (s.includes('\\') && s.includes('.xlsx'))) return '';
    return s;
  };

  const pickLine = (obj, cands, dflt = 0) => {
  ...
  (Array.isArray(lineas) ? lineas : []).forEach((l) => {
    const codigo = rejectFilePath(
      _n(pickLine(l, ['art_sku', 'art_codigo_interno', 'SKU', 'Codigo', 'Id_Articulo', 'id_articulo'], ''))
    );
    const concepto = rejectFilePath(
      _n(pickLine(l, ['art_nombre', 'art_descripcion', 'Nombre', 'Descripcion', 'Articulo', 'nombre', 'pedart_articulo_txt'], ''))
    );
    const qty = Math.max(
      0,
      toNumUtil(pickLine(l, ['Linea_Cantidad', 'pedart_cantidad', 'Cantidad', 'cantidad', 'Unidades', 'unidades', 'Uds', 'uds'], 0), 0)
    );
    const pvl = Math.max(
      0,
      toNumUtil(
        pickLine(l, ['Linea_PVP', 'pedart_pvp', 'PVP', 'pvp', 'PrecioUnitario', 'PVL', 'Precio', 'pvl', 'art_pvl'], 0),
        0
      )
    );
    const dto = Math.max(
      ...
      Math.min(
        100,
        toNumUtil(
          pickLine(l, ['Linea_Dto', 'pedart_dto', 'DtoLinea', 'dto_linea', 'Dto', 'dto', 'Descuento', 'descuento'], 0),
          0
        )
      )
    );
    let ivaPct = toNumUtil(
      pickLine(l, ['Linea_IVA', 'pedart_iva', 'IVA', 'PorcIVA', 'PorcentajeIVA', 'TipoIVA', 'art_iva'], 0),
      0
    );
    if (ivaPct > 100) ivaPct = 0;

    const baseStored = toNumUtil(
      pickLine(l, ['Linea_Subtotal', 'pedart_subtotal', 'Base', 'BaseImponible', 'Subtotal', 'Importe', 'ImporteLinea'], NaN),
      NaN
    );
    const base = Number.isFinite(baseStored)
      ? round2(baseStored)
      : round2(qty * pvl * (1 - dto / 100));
    const ivaLinea = round2(base * ivaPct / 100);
    const totalLinea = round2(base + ivaLinea);
    ...
    r.getCell(3).value = pvl;
    r.getCell(4).value = qty;
    r.getCell(5).value = dto;
    r.getCell(6).value = base;
    r.getCell(7).value = ivaPct;
    r.getCell(8).value = totalLinea;
  ...
  const importeDto = dtoPedidoPct ? round2(round2(sumBase) * dtoPedidoPct / 100) : 0;
  const baseAfterDto = round2(round2(sumBase) - importeDto);

  let ivaTotal = 0;
  const ivaPctKeys = Object.keys(ivaByPct).sort((a, b) => Number(a) - Number(b));
  const ivaRows = ivaPctKeys.map((key) => {
    const entry = ivaByPct[key];
    const ivaImporte = round2(baseAfterDto * entry.pct / 100);
    ivaTotal += ivaImporte;
    return { pct: entry.pct, importe: ivaImporte };
  });
  ivaTotal = round2(ivaTotal);
  const totalFinal = round2(baseAfterDto + ivaTotal);

  const styleTotals = (r, isGrand) => {
    r.height = 21;
    [6, 7, 8].forEach((c) => {
      const cell = r.getCell(c);
      cell.font = { name: 'Calibri', size: isGrand ? 13 : 12, bold: true, color: { argb: 'FF0F172A' } };
  ...
  const rSubtotal = ws.getRow(rowNum++);
  rSubtotal.getCell(6).value = 'SUBTOTAL';
  rSubtotal.getCell(8).value = round2(sumBase);
  rSubtotal.getCell(8).numFmt = moneyFmt;
  styleTotals(rSubtotal, false);

  if (dtoPedidoPct) {
    const rDto = ws.getRow(rowNum++);
    rDto.getCell(6).value = `DTO PEDIDO (${dtoPedidoPct.toFixed(2).replace('.', ',')}%)`;
    rDto.getCell(8).value = -importeDto;
    rDto.getCell(8).numFmt = moneyFmt;
    styleTotals(rDto, false);
  }

  const rBase = ws.getRow(rowNum++);
  rBase.getCell(6).value = 'BASE IMPONIBLE';
  rBase.getCell(8).value = baseAfterDto;
  rBase.getCell(8).numFmt = moneyFmt;
  styleTotals(rBase, false);

  for (const iv of ivaRows) {
    const rIva = ws.getRow(rowNum++);
    rIva.getCell(6).value = `${_taxLabel} (${iv.pct.toFixed(2).replace('.', ',')}%)`;
    rIva.getCell(8).value = iv.importe;
    rIva.getCell(8).numFmt = moneyFmt;
    styleTotals(rIva, false);
  }

  const rTotal = ws.getRow(rowNum++);
  rTotal.getCell(6).value = 'TOTAL';
  rTotal.getCell(8).value = totalFinal;
  rTotal.getCell(8).numFmt = moneyFmt;
  styleTotals(rTotal, true);
...
async function buildHefameXlsxBuffer({ item, id, lineas, cliente, mayoristaInfo }) {
  const numPedido = String(_n(_n(_n(item && item.NumPedido, item && item.Num_Pedido), item && item.Numero_Pedido), '')).trim();
  ...
  const wsCabecera = wb.getWorksheet('Cabecera') || wb.worksheets.find((s) => String(s.name || '').toLowerCase() === 'cabecera') || wb.worksheets[0];
  ...
  const nombre = pickCliente(cliente, ['cli_nombre_razon_social', 'Nombre_Razon_Social', 'Nombre', 'nombre']) || item?.Id_Cliente || '';
  const codigoHefame = String(_n(_n(_n(item && item.NumAsociadoHefame, item && item.num_asociado_hefame), _n(item && item.numero_cooperativa, item && item.NumeroCooperativa)), '')).trim();
  const telefono = pickCliente(cliente, ['cli_telefono', 'cli_movil', 'Telefono', 'Movil', 'Teléfono']) || '';
  const cp = String(pickCliente(cliente, ['cli_codigo_postal', 'CodigoPostal', 'codigo_postal']) || '').trim();
  const poblacion = String(pickCliente(cliente, ['cli_poblacion', 'Poblacion', 'poblacion']) || '').trim();
  const poblacionConCP = [cp, poblacion].filter(Boolean).join(' ');

  try {
    ws.getCell('F5').value = valorF5;
    ws.getCell('C13').value = nombre;
    ws.getCell('C14').value = mayoristaInfo?.codigoAsociado || codigoHefame;
    ws.getCell('C15').value = telefono;
    ws.getCell('C16').value = poblacionConCP;
    if (mayoristaInfo && mayoristaInfo.codigoAsociado) {
      wsCabecera.getCell('C14').value = mayoristaInfo.codigoAsociado;
    }
  } catch (e) {
  ...
  lineasArr.forEach((l, idx) => {
    ...
    const cantidad = Math.max(0, toNumUtil(pickLine(l, ['Linea_Cantidad', 'pedart_cantidad', 'Cantidad', 'Unidades', 'unidades'], 0), 0));
    const cn = rejectFilePath(
      _n(pickLine(l, ['art_sku', 'art_codigo_interno', 'SKU', 'Codigo', 'Id_Articulo', 'id_articulo'], ''))
    );
    const descripcion = rejectFilePath(
      _n(pickLine(l, ['art_nombre', 'art_descripcion', 'Nombre', 'Descripcion', 'Articulo', 'nombre', 'pedart_articulo_txt'], ''))
    );
    const descuentoPct = Math.max(0, Math.min(100, toNumUtil(pickLine(l, ['Linea_Dto', 'pedart_dto', 'DtoLinea', 'Dto', 'dto', 'Descuento'], 0), 0)));
  ...
  const nombreClienteRaw = cliente?.Nombre_Razon_Social || cliente?.cli_nombre_razon_social || cliente?.Nombre || '';
  const nombreClienteSafe = String(nombreClienteRaw)
    .trim()
    .replace(/\s+/g, '_')
    .replace(/[\\/:*?"<>|]/g, '')
    .slice(0, 80) || 'cliente';
  const safeNumPedido = String(numPedido).replace(/[\\/:*?"<>|]/g, '').trim() || String(id);
  const mayoristaNombreSafe = String(mayoristaInfo?.nombre || 'Hefame').replace(/[\\/:*?"<>|]/g, '').trim() || 'Transfer';
  const attachmentFileName = `${yyyymmdd}_${safeNumPedido}_${nombreClienteSafe}_Transfer_${mayoristaNombreSafe}.xlsx`;
  ...
  for (const t of (smartQ.tokens || [])) {
    ...
    if (['cliente', 'c'].includes(f)) {
      ...
      if (vv.includes('..')) {
        ...
        if (a && b) {
          per.push(`p.\`${colFecha}\` >= ? AND p.\`${colFecha}\` < ? + INTERVAL 1 DAY`);
          params.push(a, b);
        }
        ...
        if (f === 'desde') {
          ...
          per.push(`p.\`${colFecha}\` < ? + INTERVAL 1 DAY`);
          params.push(vv);
        } else {
          per.push(`p.\`${colFecha}\` >= ? AND p.\`${colFecha}\` < ? + INTERVAL 1 DAY`);
          params.push(vv, vv);
...
/**
 * Resuelve la dirección de envío de un pedido: primero por ID directo,
 * si no existe y hay cliente → usa la única dirección si solo tiene una.
 */
async function resolveDireccionEnvio(db, item, clienteId) {
  let dirEnvio = null;
  const dirId = Number(
    _n(_n(item?.Id_DireccionEnvio, item?.id_direccion_envio), item?.ped_direnv_id) || 0
  );
  if (dirId) dirEnvio = await db.getDireccionEnvioById(dirId).catch(() => null);
  if (!dirEnvio && clienteId) {
    const dirs = await db.getDireccionesEnvioByCliente(Number(clienteId), { compact: false }).catch(() => []);
    if (Array.isArray(dirs) && dirs.length === 1) dirEnvio = dirs[0];
  }
  return dirEnvio;
}
