...
async function buildStandardPedidoXlsxBuffer({ item, id, lineas, cliente, direccionEnvio, fmtDateES, mayoristaInfo }) {
  ...
  const factorDto = dtoPedidoPct ? (1 - dtoPedidoPct / 100) : 1;
  ...
  const ivaAfterDto = round2(round2(sumIva) * factorDto);
  ...
  rIva.getCell(6).value = 'IMPUESTO';
  rIva.getCell(8).value = ivaAfterDto;
