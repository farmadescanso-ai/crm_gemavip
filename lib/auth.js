...
/** Rutas que no deben comprobar com_activo (login, estáticos, health, webhooks públicos). */
function shouldSkipComercialActiveCheck(req) {
  ...
  if (p === '/sw.js' || p === '/favicon.ico') return true;
  if (p.startsWith('/webhook/')) return true;
  if (p === '/login-cliente' || p.startsWith('/login-cliente/')) return true;
  if (p.startsWith('/portal/documento/')) return true;
  if (p === '/logout-portal') return true;
  return false;
}

/**
 * Si hay sesión, el comercial debe existir y estar activo (com_activo=1).
 * Si no: cierra sesión y redirige a login o 403 JSON.
 */
function requireSessionComercialActive(req, res, next) {
  if (!req.session?.user?.id) return next();
  if (shouldSkipComercialActiveCheck(req)) return next();

  const id = Number(req.session.user.id);
  if (!Number.isFinite(id) || id <= 0) return next();

  db.isComercialActiveById(id)
    .then((ok) => {
      if (ok) return next();
      req.session.destroy((err) => {
        ...
        if (accept.includes('application/json')) {
          return res.status(403).json({ ok: false, error: 'cuenta_inactiva', message: 'Cuenta desactivada' });
        }
        return res.redirect('/login?cuenta=inactiva');
      });
    })
    .catch((e) => next(e));
}
...
function getRoleNavLinksForRoles(roles, user) {
  ...
  if (isAdmin) links.push({ href: '/admin/sql-centro-murciasalud', label: 'Centros prescriptores' });
  ...
  if (isAdmin) links.push({ href: '/admin/portal-config', label: 'Portal cliente' });
