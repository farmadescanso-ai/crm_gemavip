...
function buildClienteFormModel({ mode, meta, item, comerciales, tarifas, provincias, paises, formasPago, tiposClientes, idiomas, monedas, estadosCliente, cooperativas, gruposCompras, canChangeComercial, missingFields }) {
  ...
  /* Tipo Contacto (Empresa/Persona) eliminado: duplicado */
  ignore.add('TipoContacto');
  ignore.add('tipo_contacto');
  ignore.add('cli_tipo_contacto');
  ...
  const labelize = (name) => {
    ...
    const overrides = {
      id_estdocliente: 'Tipo Contacto',
      id_estadocliente: 'Tipo Contacto',
      cli_estcli_id: 'Tipo Contacto',
      ok_ko: 'Estado',
      cli_ok_ko: 'Estado',
      id_cial: 'Delegado',
  ...
  const toTab = (name) => {
    ...
    if (['nombre_razon_social', 'nombre_cial', 'dni_cif', 'ok_ko', 'id_estdocliente', 'id_estadocliente', 'cli_nombre_razon_social', 'cli_nombre_cial', 'cli_dni_cif', 'cli_ok_ko', 'cli_estcli_id', 'id_tipocliente', 'id_tipo_cliente', 'cli_tipc_id'].includes(n)) return 'ident';
  ...
  byId.get('ident').fields = promote(byId.get('ident').fields, ['Nombre_Razon_Social', 'Nombre_Cial', 'cli_nombre_razon_social', 'cli_nombre_cial', 'Id_TipoCliente', 'cli_tipc_id', 'Id_EstdoCliente', 'cli_estcli_id', 'DNI_CIF', 'cli_dni_cif', 'OK_KO', 'cli_ok_ko']);
...
function resolveDisplayValue(name, item, catalogs = {}) {
  if (!item) return '';
  const n = String(name || '').toLowerCase();
  const raw = (item[name] !== undefined) ? item[name] : '';
