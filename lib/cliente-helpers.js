...
function applySpainDefaultsIfEmpty(item, { meta, paises, idiomas, monedas } = {}) {
  ...
  if (aplicarIdiomaMonedaDefault && (hasCol('Id_Moneda') || hasCol('cli_mon_id')) && monedaVacia && (monedas || []).length) {
    ...
    if (eurId) {
      if (hasCol('cli_mon_id')) item.cli_mon_id = eurId;
      if (hasCol('Id_Moneda')) item.Id_Moneda = eurId;
    }
  }
...
function buildClienteFormModel({ mode, meta, item, comerciales, tarifas, provincias, paises, formasPago, tiposClientes, especialidades, idiomas, monedas, estadosCliente, cooperativas, gruposCompras, canChangeComercial, missingFields }) {
  ...
  if (item && (mode === 'create' || mode === 'edit') && monedas?.length) {
    const hasMonFld = colsLower.has('cli_mon_id') || colsLower.has('id_moneda');
    if (hasMonFld) {
      ...
      if (empty) {
        const eurId = resolveMonedaEurId(monedas);
        if (eurId) {
          if (colsLower.has('cli_mon_id')) item.cli_mon_id = eurId;
          if (colsLower.has('id_moneda')) item.Id_Moneda = eurId;
        }
      }
    }
  }
