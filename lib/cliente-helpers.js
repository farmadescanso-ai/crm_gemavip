...
function applySpainDefaultsIfEmpty(item, { meta, paises, idiomas, monedas } = {}) {
  ...
  if (isClienteEspana && (hasCol('Id_Moneda') || hasCol('cli_mon_id')) && monedaVacia) {
    const direct =
    ...
    if (eurId) {
      item.Id_Moneda = eurId;
      item.cli_mon_id = eurId;
    }
  }
...
function buildClienteFormModel({ mode, meta, item, comerciales, tarifas, provincias, paises, formasPago, tiposClientes, especialidades, idiomas, monedas, estadosCliente, cooperativas, gruposCompras, canChangeComercial, missingFields }) {
  ...
  const toTab = (name) => {
    ...
    if (n === 'cli_idiom_id' || n === 'id_idioma' || n === 'cli_mon_id' || n === 'id_moneda') return 'avanzado';
  ...
  byId.get('condiciones').fields = promote(byId.get('condiciones').fields, [meta?.colComercial || 'Id_Cial', 'cli_com_id', 'Tarifa', 'cli_tarcli_id', 'cli_tarifa_legacy', 'Dto', 'cli_dto', 'Id_FormaPago', 'cli_formp_id']);
  byId.get('avanzado').fields = promote(byId.get('avanzado').fields, ['cli_idiom_id', 'Id_Idioma', 'cli_mon_id', 'Id_Moneda', 'cli_RE', 'cli_re']);
