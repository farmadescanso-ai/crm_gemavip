...
const TABLE_LABEL_MAP = {
  tipos_clientes: ['tipc_nombre', 'tipc_tipo', 'Tipo', 'Nombre', 'nombre'],
...
async function loadEstadosClienteForSelect(db) {
  try {
    ...
    const colsLower = new Set((cols || []).map((c) => String(c).toLowerCase()));
    const pkCol = colsLower.has('estcli_id') ? 'estcli_id' : (colsLower.has('id') ? 'id' : 'Id');
    const nomCands = ['estcli_nombre', 'Nombre', 'nombre', 'Estado', 'estado', 'descripcion', 'Descripcion'];
    const nomCol = nomCands.find((c) => colsLower.has(c)) || 'Nombre';
...
function buildClienteFormModel({ mode, meta, item, comerciales, tarifas, provincias, paises, formasPago, tiposClientes, especialidades, idiomas, monedas, estadosCliente, cooperativas, gruposCompras, canChangeComercial, missingFields }) {
  ...
  const toTab = (name) => {
    ...
    if (['nombre_razon_social', 'nombre_cial', 'dni_cif', 'ok_ko', 'id_estdocliente', 'id_estadocliente', 'cli_nombre_razon_social', 'cli_nombre_cial', 'cli_dni_cif', 'cli_ok_ko', 'cli_estcli_id', 'id_tipocliente', 'id_tipo_cliente', 'cli_tipc_id', 'cli_esp_id', 'tipocontacto', 'tipo_contacto', 'cli_tipo_contacto'].includes(n)) return 'ident';
  ...
  byId.get('ident').fields = promote(byId.get('ident').fields, ['Nombre_Razon_Social', 'Nombre_Cial', 'cli_nombre_razon_social', 'cli_nombre_cial', 'Id_TipoCliente', 'cli_tipc_id', 'cli_esp_id', 'TipoContacto', 'cli_tipo_contacto', 'Id_EstdoCliente', 'cli_estcli_id', 'DNI_CIF', 'cli_dni_cif', 'OK_KO', 'cli_ok_ko']);
