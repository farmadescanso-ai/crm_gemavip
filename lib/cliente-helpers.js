...
function buildClienteFormModel({ mode, meta, item, comerciales, tarifas, provincias, paises, formasPago, tiposClientes, idiomas, monedas, estadosCliente, cooperativas, gruposCompras, canChangeComercial, missingFields }) {
  ...
  const toTab = (name) => {
    ...
    if (n === 'id_tipocliente' || n === 'id_tipo_cliente' || n === 'cli_tipc_id') return 'ident';
  ...
  byId.get('ident').fields = promote(byId.get('ident').fields, ['Nombre_Razon_Social', 'Nombre_Cial', 'cli_nombre_razon_social', 'cli_nombre_cial', 'TipoContacto', 'cli_tipo_contacto', 'DNI_CIF', 'cli_dni_cif', 'OK_KO', 'cli_ok_ko', 'Id_EstdoCliente', 'cli_estcli_id', 'Id_TipoCliente', 'cli_tipc_id']);
