...
function buildClienteFormModel({ mode, meta, item, comerciales, tarifas, provincias, paises, formasPago, tiposClientes, especialidades, idiomas, monedas, estadosCliente, cooperativas, gruposCompras, canChangeComercial, missingFields }) {
  ...
  const toTab = (name) => {
    ...
    if (n.includes('tarifa') || n === 'dto' || n.includes('descuento') || n.includes('comercial') || n.includes('id_cial') || n.includes('com_id') || n.includes('formp_id')) return 'condiciones';
  ...
  byId.get('condiciones').fields = promote(byId.get('condiciones').fields, [meta?.colComercial || 'Id_Cial', 'cli_com_id', 'Tarifa', 'cli_tarcli_id', 'cli_tarifa_legacy', 'Id_FormaPago', 'cli_formp_id', 'cli_idiom_id', 'Id_Idioma', 'cli_mon_id', 'Id_Moneda', 'Dto', 'cli_dto']);
  byId.get('avanzado').fields = promote(byId.get('avanzado').fields, ['cli_RE', 'cli_re']);
