...
function buildClienteFormModel({ mode, meta, item, comerciales, tarifas, provincias, paises, formasPago, tiposClientes, especialidades, idiomas, monedas, estadosCliente, cooperativas, gruposCompras, canChangeComercial, missingFields, isAdmin = true }) {
  const cols = Array.isArray(meta?.cols) ? meta.cols : [];
  const pk = meta?.pk || 'Id';
  ...
  const tabs = [
    ...
    ...(mode === 'edit' || mode === 'view' ? [{ id: 'mayoristas', label: 'Mayoristas', fields: [{ name: '_mayoristas', label: '', spec: { kind: 'mayoristas' } }] }] : []),
