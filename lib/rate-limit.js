...
const portalLoginLimiter = rateLimit({
  ...
  standardHeaders: true,
  legacyHeaders: false,
  message: { error: 'Demasiados intentos de acceso al portal. Inténtalo en 15 minutos.' },
  handler(req, res, _next, options) {
    const wantsJson = /application\/json/i.test(req.headers.accept || '');
    if (wantsJson) return res.status(429).json(options.message);
    return res.status(429).render('portal/login-cliente', {
      title: 'Portal cliente',
      error: options.message?.error || 'Demasiados intentos.',
      returnTo: null,
      restablecido: false,
...
const portalPasswordResetLimiter = rateLimit({
  windowMs: 15 * 60 * 1000,
  max: 3,
  standardHeaders: true,
  legacyHeaders: false,
...
module.exports = { loginLimiter, apiLimiter, passwordResetLimiter, portalLoginLimiter, portalPasswordResetLimiter };
