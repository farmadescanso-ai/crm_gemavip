 ...
 * @param {{ skipProvinciaFilter?: boolean, provincia?: string, provinciaNombre?: string }} opts
 */
function passesProvinciaFilter(provinceStr, opts) {
  if (opts.skipProvinciaFilter) return true;
  const needle = String(opts.provinciaNombre || opts.provincia || 'Murcia').trim().toLowerCase();
  if (!needle) return true;
  return String(provinceStr || '').toLowerCase().includes(needle);
}
 ...
 */
async function getContactCached(apiKey, contactId, cache) {
  const k = String(contactId);
  if (cache && cache.has(k)) return cache.get(k);
  const contact = await fetchHolded(apiKey, 'GET', `/contacts/${contactId}`);
  if (cache) cache.set(k, contact);
  return contact;
}

/**
 * @param {boolean} opts.strictClienteCrm — solo cli_Id_Holded / cli_referencia; sin CIF ni alta.
 */
async function resolveClienteIdForHoldedContact(db, contactId, contact, opts) {
  const strict = !!opts.strictClienteCrm;
  const allowRef = opts.allowLegacyReferencia !== false;

  let rows = await db.query('SELECT cli_id FROM clientes WHERE cli_Id_Holded = ? LIMIT 1', [contactId]).catch(() => []);
  if (rows?.length) return rows[0].cli_id;

  if (allowRef) {
    rows = await db.query('SELECT cli_id FROM clientes WHERE cli_referencia = ? LIMIT 1', [contactId]).catch(() => []);
    if (rows?.length) return rows[0].cli_id;
  }

  if (strict) return null;

  const cif = String(contact?.code ?? '').trim();
  if (cif) {
    const byCif = await db.query('SELECT cli_id FROM clientes WHERE cli_dni_cif = ? LIMIT 1', [cif]).catch(() => []);
    if (byCif?.length) {
      const cid = byCif[0].cli_id;
      try {
        await db.query('UPDATE clientes SET cli_Id_Holded = ? WHERE cli_id = ?', [contactId, cid]);
      } catch (_) {
        await db.query('UPDATE clientes SET cli_referencia = ? WHERE cli_id = ?', [contactId, cid]).catch(() => {});
      }
      return cid;
    }
  }
...
/**
 * @returns {Promise<{ ok: boolean, lineIndex?: number }>}
 */
async function allProductLinesMatchArticulo(products, artIdDefault, db) {
  ...
  for (let i = 0; i < list.length; i++) {
    ...
    const resolved = artId != null ? Number(artId) : null;
    const def = artIdDefault != null ? Number(artIdDefault) : null;
    if (resolved == null || resolved === def) {
      return { ok: false, lineIndex: i + 1 };
    }
  }
  return { ok: true };
}
...
async function runSyncHoldedPedidos(opts = {}) {
  ...
  const skipProvinciaFilter = !!opts.skipProvinciaFilter;
  const provinciaNombre = opts.provincia || 'Murcia';
  const strictClienteCrm = !!opts.strictClienteCrm;
  const autoCreateCliente = opts.autoCreateCliente !== false;
  const requireFullLineMatch = !!opts.requireFullLineMatch;
  const allowLegacyReferencia = opts.allowLegacyReferencia !== false;
  ...
  const filterOpts = { skipProvinciaFilter, provinciaNombre };
  const resolveOpts = { strictClienteCrm, allowLegacyReferencia };
  ...
  try {
    ...
    const provincias = await db.query('SELECT prov_id, prov_nombre, prov_codigo_pais FROM provincias WHERE prov_nombre LIKE ?', [`%${provinciaNombre}%`]);
    const provDefaultId = provincias?.[0]?.prov_id ?? PROVINCIA_MURCIA_ID;
    ...
    for (const doc of docsToProcess) {
      ...
      let contact;
      try {
        contact = await getContactCached(apiKey, contactId, contactCache);
      } catch (e) {
        result.skippedSinContacto++;
        continue;
      }
      ...
      if (!passesProvinciaFilter(province, filterOpts)) {
      ...
      let clienteId = await resolveClienteIdForHoldedContact(db, contactId, contact, resolveOpts);

      if (!clienteId && autoCreateCliente && !strictClienteCrm) {
      ...
      if (requireFullLineMatch && products.length) {
        const lineCheck = await allProductLinesMatchArticulo(products, artIdDefault, db);
        if (!lineCheck.ok) {
          result.skippedLineasSinMatch++;
          continue;
        }
      }
 ...
 * @param {Object} opts - { start, end, provincia, skipProvinciaFilter, strictClienteCrm, requireFullLineMatch, allowLegacyReferencia }
 * @returns {Promise<{ ok: boolean, pedidos: Array, totalFetched, skippedProvincia, skippedDuplicado, skippedSinContacto, skippedSinClienteCRM, skippedLineasSinMatch, periodo, error?: string }>}
 */
async function getPreviewPedidosHolded(opts = {}) {
  ...
  try {
    ...
    const artDefaultRows = await db.query('SELECT art_id FROM articulos ORDER BY art_id ASC LIMIT 1');
    const artIdDefaultRaw = artDefaultRows?.length && artDefaultRows[0].art_id != null ? Number(artDefaultRows[0].art_id) : null;
    const artIdDefault = artIdDefaultRaw > 0 ? artIdDefaultRaw : 1;
    ...
    if (!Array.isArray(documents)) {
      return {
        ...
        skippedLineasSinMatch: 0,
        periodo: `${start} a ${end}`,
        error: 'Holded no devolvió array'
      };
    }
    ...
    let skippedSinClienteCRM = 0;
    let skippedLineasSinMatch = 0;
    ...
    for (const doc of documents) {
      ...
      let clienteId = await resolveClienteIdForHoldedContact(db, contactId, contact, resolveOpts);
      /** En modo estricto sin alta automática (CPanel): no listar pedidos de contactos sin fila CRM vinculada. */
      const tendriaCliente =
        clienteId != null || (autoCreateCliente && !strictClienteCrm);
      if (!tendriaCliente) {
        skippedSinClienteCRM++;
        continue;
      }
      ...
      if (yaExiste) {
        skippedDuplicado++;
        if (!motivoExclusion) motivoExclusion = 'ya_en_bd';
      }
      ...
      if (requireFullLineMatch && products.length) {
        const lineCheck = await allProductLinesMatchArticulo(products, artIdDefault, db);
        lineasOk = lineCheck.ok;
        if (!lineCheck.ok) {
          skippedLineasSinMatch++;
          if (!motivoExclusion) motivoExclusion = 'lineas_sin_match';
        }
      }

      const puedeImportar = !yaExiste && lineasOk;
      ...
      pedidos.push({
        ...
        cliId: clienteId ?? null,
        yaExiste,
        estado: mapHoldedStatus(doc.status),
        motivoExclusion,
        puedeImportar,
        lineasCatalogoOk: lineasOk
      });
    }
