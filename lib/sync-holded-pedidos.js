    ...
    for (const doc of docsToProcess) {
      ...
      const [pedResult] = await db.pool.execute(
        `INSERT INTO pedidos (
          ...
          ped_total, ped_base, ped_iva, ped_dto, ped_id_holded
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        [1, clienteId, formpId, tippId, 'A', pedNumero, pedFecha, estadoTxt, doc.total ?? 0, doc.subtotal ?? 0, doc.tax ?? 0, pedDto, doc.id]
      );
