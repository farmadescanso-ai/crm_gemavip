...
/** Webhook N8N para emails de sincronización Holded (sin depender de SMTP @gemavip). Sobrescribe con HOLDED_SYNC_N8N_WEBHOOK_URL; usa `0` para desactivar. */
const HOLDED_SYNC_N8N_WEBHOOK_DEFAULT =
  'https://farmadescanso-n8n.6f4r35.easypanel.host/webhook/58663207-04f0-4a20-b333-1bd4ff36bf00';

function resolveHoldedSyncN8nWebhookUrl() {
  const raw = process.env.HOLDED_SYNC_N8N_WEBHOOK_URL;
  if (raw !== undefined && String(raw).trim() !== '') {
    const s = String(raw).trim().toLowerCase();
    if (s === '0' || s === 'false' || s === 'off' || s === 'disabled') return '';
    return String(raw).trim();
  }
  return HOLDED_SYNC_N8N_WEBHOOK_DEFAULT;
}

/** CC para notificaciones Sync Holded (coma o `;`). Por defecto `paco@fralabu.com`. `HOLDED_SYNC_NOTIFY_CC=0` desactiva copia. */
function resolveHoldedSyncNotifyCc() {
  const raw = process.env.HOLDED_SYNC_NOTIFY_CC;
  if (raw !== undefined && String(raw).trim() !== '') {
    const s = String(raw).trim();
    if (s === '0' || s.toLowerCase() === 'none' || s.toLowerCase() === 'off') return [];
    return s.split(/[,;]/).map((a) => a.trim()).filter(Boolean);
  }
  return ['paco@fralabu.com'];
}

/**
 * Envía payload JSON a N8N para que envíe el correo (u otra automatización).
 * @param {{ event: string, to: string, subject: string, text?: string, html?: string, cc?: string|string[], meta?: Record<string, unknown> }} body
 * @returns {Promise<{ sent: boolean, via?: string, error?: string, skipped?: boolean }>}
 */
async function postHoldedSyncToN8nWebhook(body) {
  const url = resolveHoldedSyncN8nWebhookUrl();
  ...
  const secret = String(
    process.env.HOLDED_SYNC_N8N_WEBHOOK_SECRET || process.env.APROBACION_SECRET || process.env.API_KEY || ''
  ).trim();
  ...
  if (secret) {
    ...
    headers['X-CRM-Signature-Alg'] = 'HMAC-SHA256';
  }
  try {
    ...
    if (resp.status >= 200 && resp.status < 300) {
      console.log('[MAILER] Holded sync → N8N webhook OK', body.event, resp.status);
      return { sent: true, via: 'n8n' };
    }
    const snippet =
      typeof resp.data === 'string' ? resp.data : JSON.stringify(resp.data || {});
    console.warn('[MAILER] Holded sync N8N webhook HTTP', resp.status, snippet.slice(0, 400));
    return { sent: false, error: `N8N HTTP ${resp.status}` };
  } catch (e) {
    console.warn('[MAILER] Holded sync N8N webhook error:', e?.message || e);
...
/**
 * Lista de clientes con cambios CRM pendientes de volcar a Holded (notificación a dirección).
 * @param {string} to
 * @param {Record<string, unknown>[]} clientesRows
 * @returns {Promise<{ sent: boolean, error?: string }>}
 */
async function sendHoldedSyncPendingDigestEmail(to, clientesRows = []) {
  const n = Array.isArray(clientesRows) ? clientesRows.length : 0;
  const subject = `CRM Gemavip: clientes pendientes de sincronizar con Holded (${n})`;
  const lines = (Array.isArray(clientesRows) ? clientesRows : []).map((c) => {
    const id = c.cli_id ?? c.Id ?? c.id;
    const nom = String(c.cli_nombre_razon_social ?? c.Nombre_Razon_Social ?? '—').trim();
    const cif = String(c.cli_dni_cif ?? c.DNI_CIF ?? '—').trim();
    const hid = String(c.cli_Id_Holded ?? c.cli_referencia ?? '—').trim();
    return `- #${id} ${nom} · CIF/NIF: ${cif} · Holded: ${hid}`;
  });
  const text = `Hay ${n} contacto(s) con datos guardados en el CRM que aún no se han volcado a Holded (revisar CPanel Holded o exportar contacto).\n\n${lines.join('\n')}`;
  const rowsHtml = (Array.isArray(clientesRows) ? clientesRows : [])
    .map((c) => {
      const id = c.cli_id ?? c.Id ?? c.id;
      const nom = escapeHtml(String(c.cli_nombre_razon_social ?? c.Nombre_Razon_Social ?? '—').trim());
      const cif = escapeHtml(String(c.cli_dni_cif ?? c.DNI_CIF ?? '—').trim());
      const hid = escapeHtml(String(c.cli_Id_Holded ?? c.cli_referencia ?? '—').trim());
      return `<tr><td style="padding:8px 12px;border-bottom:1px solid #e2e8f0;">#${escapeHtml(String(id))}</td><td style="padding:8px 12px;border-bottom:1px solid #e2e8f0;">${nom}</td><td style="padding:8px 12px;border-bottom:1px solid #e2e8f0;">${cif}</td><td style="padding:8px 12px;border-bottom:1px solid #e2e8f0;">${hid}</td></tr>`;
    })
    .join('');
...
<body style="margin:0;padding:0;background:#f4f6f9;font-family:'Segoe UI',Roboto,Arial,sans-serif;">
  <table width="100%" cellpadding="0" cellspacing="0" style="background:#f4f6f9;padding:28px 0;">
    <tr><td align="center">
      <table width="640" cellpadding="0" cellspacing="0" style="background:#fff;border-radius:12px;box-shadow:0 2px 8px rgba(0,0,0,0.08);overflow:hidden;border-top:4px solid #0ea5e9;">
        <tr><td style="padding:24px 28px;">
          <h1 style="margin:0 0 8px;font-size:18px;color:#0f172a;">Clientes pendientes de sincronizar con Holded</h1>
          <p style="margin:0 0 16px;font-size:14px;color:#64748b;line-height:1.5;">${n} contacto(s) con cambios en el CRM respecto al último volcado acordado con Holded. Revisa el CPanel o exporta los contactos a Holded.</p>
          <table width="100%" cellpadding="0" cellspacing="0" style="border:1px solid #e2e8f0;border-radius:8px;overflow:hidden;font-size:13px;">
            <thead><tr style="background:#f8fafc;text-align:left;">
              ...
              <th style="padding:8px 12px;color:#475569;">Nombre</th>
              <th style="padding:8px 12px;color:#475569;">CIF/NIF</th>
              <th style="padding:8px 12px;color:#475569;">Holded</th>
            </tr></thead>
            <tbody>${rowsHtml}</tbody>
          </table>
          <p style="margin:16px 0 0;font-size:12px;color:#94a3b8;">Correo automático · CRM Gemavip</p>
        </td></tr>
      </table>
    </td></tr>
  </table>
</body></html>`;
  const ccList = resolveHoldedSyncNotifyCc();
  const ccStr = ccList.length ? ccList.join(', ') : '';
  const n8n = await postHoldedSyncToN8nWebhook({
    event: 'holded_sync_digest',
    to,
    subject,
    text,
    html,
    ...
    meta: {
      count: n,
      clientes: (Array.isArray(clientesRows) ? clientesRows : []).slice(0, 500).map((c) => ({
        cli_id: c.cli_id ?? c.Id ?? c.id,
        nombre: String(c.cli_nombre_razon_social ?? c.Nombre_Razon_Social ?? '').trim(),
        cif: String(c.cli_dni_cif ?? c.DNI_CIF ?? '').trim(),
        holdedId: String(c.cli_Id_Holded ?? c.cli_referencia ?? '').trim()
      }))
    }
  });
  if (n8n.sent) return n8n;
  return sendPedidoEmail(to, { subject, text, html, cc: ccList.length ? ccList : undefined });
}

function computeSigSyncCliente(notifId, accion) {
  const secret = (process.env.APROBACION_SECRET || process.env.API_KEY || 'crm-gemavip-aprobacion').trim();
  return crypto.createHmac('sha256', secret).update(`notifId=${notifId}&accion=${accion}`).digest('hex');
}

/**
 * Email con enlaces firmados para decidir sincronización Holded ↔ CRM (dirección).
 * @param {string} to
 * @param {{ notifId: number, cliId: number, clienteNombre?: string, diffCampos?: string[], sugerencia?: string|null, pteExportar?: boolean, pteImportar?: boolean }} data
 */
async function sendHoldedSyncApprovalRequestEmail(to, data = {}) {
  const notifId = Number(data.notifId);
  if (!Number.isFinite(notifId) || notifId <= 0) return { sent: false, error: 'notifId inválido' };
  const cliId = Number(data.cliId);
  const nombre = escapeHtml(String(data.clienteNombre || (Number.isFinite(cliId) ? `Cliente #${cliId}` : 'Cliente')).trim());
  const diffs = Array.isArray(data.diffCampos) ? data.diffCampos.slice(0, 30) : [];
  const sug = data.sugerencia || null;
  const pteEx = data.pteExportar === true;
  const pteIm = data.pteImportar === true;
  const base = APP_BASE_URL.replace(/\/$/, '');
  /** Solo dos acciones cuando el caso es claro (evita ofrecer Holded→CRM tras editar en CRM, o al revés). */
  let acciones;
  if (pteEx && !pteIm) {
    acciones = [
      { key: 'crm_to_holded', label: 'Aplicar CRM → Holded' },
      { key: 'revisar', label: 'Rechazar y revisar luego' }
    ];
  } else if (pteIm && !pteEx) {
    acciones = [
      { key: 'holded_to_crm', label: 'Traer Holded → CRM' },
      { key: 'revisar', label: 'Rechazar y revisar luego' }
    ];
  } else {
    acciones = [
  ...
  const links = acciones.map((a) => {
    const sig = computeSigSyncCliente(notifId, a.key);
    const url = `${base}/webhook/aprobar-sync-cliente?notifId=${notifId}&accion=${encodeURIComponent(a.key)}&sig=${sig}`;
    return { key: a.key, label: a.label, url };
  });
  const diffBlock =
    diffs.length > 0
      ? `<ul style="margin:8px 0;padding-left:20px;font-size:13px;color:#475569;">${diffs.map((d) => `<li>${escapeHtml(String(d))}</li>`).join('')}</ul>`
      : '<p style="font-size:13px;color:#64748b;">(Sin detalle de campos; revisar en CRM.)</p>';
  const sugTxt =
    pteEx && !pteIm
      ? 'Los datos guardados en el CRM difieren de la última copia acordada con Holded: puede volcar el CRM → Holded o posponer.'
      : pteIm && !pteEx
        ? 'Holded tiene cambios respecto a la última copia acordada; el CRM aún no refleja esa edición: puede traer Holded → CRM o posponer.'
        : sug === 'crm_to_holded'
          ? 'Sugerencia: volcar datos del CRM a Holded.'
          : sug === 'holded_to_crm'
            ? 'Sugerencia: actualizar el CRM con lo que hay en Holded.'
            : 'Hay divergencia entre sistemas: elija la acción o revise en el CRM.';
  const subject = `Decidir sincronización Holded · ${nombre.replace(/<[^>]+>/g, '') || 'Cliente'}`;
  const html = `<!DOCTYPE html>
<html lang="es"><head><meta charset="UTF-8"></head>
<body style="margin:0;padding:0;background:#f4f6f9;font-family:'Segoe UI',Roboto,Arial,sans-serif;">
  <table width="100%" cellpadding="0" cellspacing="0" style="background:#f4f6f9;padding:28px 0;">
    <tr><td align="center">
      <table width="600" cellpadding="0" cellspacing="0" style="background:#fff;border-radius:12px;box-shadow:0 2px 8px rgba(0,0,0,0.08);border-top:4px solid #0ea5e9;">
        <tr><td style="padding:24px 28px;">
          <h1 style="margin:0 0 8px;font-size:18px;color:#0f172a;">Sincronización Holded ↔ CRM</h1>
          <p style="margin:0 0 12px;font-size:14px;color:#64748b;">${escapeHtml(sugTxt)}</p>
          <p style="margin:0 0 8px;font-size:14px;"><strong>Cliente:</strong> ${nombre}</p>
          ${Number.isFinite(cliId) ? `<p style="margin:0 0 12px;font-size:13px;color:#64748b;">ID CRM: ${cliId} · Notificación #${notifId}</p>` : ''}
          <p style="margin:0 0 4px;font-size:13px;font-weight:600;color:#334155;">Campos distintos (resumen)</p>
          ${diffBlock}
          <div style="margin-top:20px;display:flex;flex-direction:column;gap:10px;">
            ${links
              .map(
                (l) =>
                  `<a href="${escapeHtml(l.url)}" style="display:inline-block;padding:12px 18px;background:#0ea5e9;color:#fff;text-decoration:none;border-radius:8px;font-weight:600;text-align:center;">${escapeHtml(l.label)}</a>`
              )
              .join('')}
          </div>
          <p style="margin:16px 0 0;font-size:11px;color:#94a3b8;">Enlaces firmados; caducidad según política del servidor.</p>
          <p style="margin:12px 0 0;font-size:11px;color:#64748b;line-height:1.45;">Al pulsar un botón se abre una página del CRM: haz clic en <strong>Confirmar</strong> para aplicar la acción (así los rastreadores o la previsualización de enlaces no consumen la solicitud antes de tiempo).</p>
        </td></tr>
      </table>
    </td></tr>
  </table>
  ...
  const ccStr = ccList.length ? ccList.join(', ') : '';
  const n8n = await postHoldedSyncToN8nWebhook({
    event: 'holded_sync_approval_request',
    to,
    subject,
    html,
    ...(ccStr ? { cc: ccStr } : {}),
    meta: {
      notifId,
      cliId: Number.isFinite(cliId) ? cliId : null,
      clienteNombre: String(data.clienteNombre || '').trim() || null,
      diffCampos: diffs.map((d) => String(d)),
      sugerencia: sug,
      links
    }
  });
  if (n8n.sent) return n8n;
  return sendPedidoEmail(to, { subject, html, cc: ccList.length ? ccList : undefined });
}

/**
 * Aviso a operativa cuando Holded se actualiza tras una decisión de sync.
 * @param {string} to
 * @param {{ accion: string, cliId: number, clienteNombre?: string, holdedId?: string }} data
 */
async function sendHoldedSyncAppliedNotifyEmail(to, data = {}) {
  const accion = String(data.accion || '').trim();
  const cliId = Number(data.cliId);
  const nom = String(data.clienteNombre || '').trim();
  const hid = String(data.holdedId || '').trim();
  const label =
    accion === 'crm_to_holded'
      ? 'Se aplicó CRM → Holded (PUT contacto).'
      : accion === 'holded_to_crm'
        ? 'Se aplicó Holded → CRM (actualización desde API).'
        : 'Sincronización completada.';
  const subject = `Holded actualizado · ${nom ? nom.slice(0, 60) : `#${cliId}`}`;
  const html = `<!DOCTYPE html>
<html lang="es"><head><meta charset="UTF-8"></head>
<body style="margin:0;padding:0;background:#f4f6f9;font-family:Segoe UI,Roboto,Arial,sans-serif;">
  <table width="100%" cellpadding="0" cellspacing="0" style="background:#f4f6f9;padding:24px 0;">
    <tr><td align="center">
      <table width="520" style="background:#fff;border-radius:12px;padding:24px;border-top:4px solid #8fae1b;">
        <tr><td>
          <h1 style="margin:0 0 12px;font-size:17px;color:#0f172a;">${escapeHtml(label)}</h1>
          <p style="margin:0;font-size:14px;color:#475569;">Cliente CRM: <strong>${Number.isFinite(cliId) ? cliId : '—'}</strong></p>
          ${hid ? `<p style="margin:8px 0 0;font-size:14px;color:#475569;">Holded ID: <strong>${escapeHtml(hid)}</strong></p>` : ''}
        </td></tr>
      </table>
    </td></tr>
  </table>
...
/**
 * Aviso cuando un contacto se crea o se modifica desde el formulario del CRM.
 * Destinatario: `CLIENTE_MODIFICADO_MAIL_TO` o por defecto p.lara@gemavip.com.
 * @param {{ accion?: 'alta'|'edicion', clienteId: number, clienteNombre?: string, editorEmail?: string, editorNombre?: string, fichaUrl?: string }} data
 */
async function sendClienteModificadoEmail(data = {}) {
  const to = String(process.env.CLIENTE_MODIFICADO_MAIL_TO || 'p.lara@gemavip.com').trim();
  if (!to) return { sent: false, error: 'Sin destinatario' };
  const esAlta = data.accion === 'alta' || data.accion === 'create';
  const id = Number(data.clienteId);
  const idOk = Number.isFinite(id) && id > 0;
  const nombreRaw = String(data.clienteNombre || '').trim();
  const nombre = escapeHtml(nombreRaw || (idOk ? `Cliente #${id}` : '—'));
  const editorNombre = String(data.editorNombre || '').trim();
  const editorEmail = String(data.editorEmail || '').trim();
  const lineaEditor =
    editorNombre && editorEmail && editorNombre.toLowerCase() !== editorEmail.toLowerCase()
      ? `${editorNombre} · ${editorEmail}`
      : editorNombre || editorEmail || 'Usuario CRM';
  const quien = escapeHtml(lineaEditor);
  const fichaUrl = String(data.fichaUrl || (idOk ? `${APP_BASE_URL.replace(/\/$/, '')}/clientes/${id}` : APP_BASE_URL)).trim();
  const verbo = esAlta ? 'creado' : 'modificado';
  const etiquetaQuien = esAlta ? 'Creado por' : 'Modificado por';
  const subject = idOk
    ? `Contacto ${verbo} · ${nombreRaw ? nombreRaw.slice(0, 80) : '#' + id} · CRM Gemavip`
    : `Contacto ${verbo} · CRM Gemavip`;
  const text = [
    esAlta ? 'Se ha dado de alta un contacto nuevo en el CRM.' : 'Se ha guardado un contacto desde el CRM.',
    idOk ? `ID: ${id}` : '',
    nombreRaw ? `Nombre: ${nombreRaw}` : '',
    `${etiquetaQuien}: ${editorNombre || editorEmail || '—'}`,
...
<body style="margin:0;padding:0;background:#f4f6f9;font-family:'Segoe UI',Roboto,Arial,sans-serif;">
  <table width="100%" cellpadding="0" cellspacing="0" style="background:#f4f6f9;padding:28px 0;">
    <tr><td align="center">
      <table width="520" cellpadding="0" cellspacing="0" style="background:#fff;border-radius:12px;box-shadow:0 2px 8px rgba(0,0,0,0.08);overflow:hidden;border-top:4px solid #0ea5e9;">
        <tr><td style="padding:28px 28px 20px;">
          <h1 style="margin:0 0 12px;font-size:18px;color:#0f172a;">Contacto ${esAlta ? 'nuevo en el CRM' : 'modificado en el CRM'}</h1>
          <p style="margin:0 0 16px;font-size:14px;color:#64748b;line-height:1.5;">${esAlta ? 'Se ha registrado un contacto nuevo desde el formulario de alta.' : 'Se han guardado cambios en un contacto desde el formulario de edición.'}</p>
          <table width="100%" cellpadding="0" cellspacing="0" style="background:#f8fafc;border-radius:8px;border:1px solid #e2e8f0;font-size:13px;">
            ${idOk ? `<tr><td style="padding:10px 14px;color:#64748b;border-bottom:1px solid #e2e8f0;width:120px;">ID</td><td style="padding:10px 14px;font-weight:700;color:#1e293b;border-bottom:1px solid #e2e8f0;">${escapeHtml(String(id))}</td></tr>` : ''}
            <tr><td style="padding:10px 14px;color:#64748b;border-bottom:1px solid #e2e8f0;vertical-align:top;">Nombre</td><td style="padding:10px 14px;font-weight:700;color:#1e293b;border-bottom:1px solid #e2e8f0;">${nombre}</td></tr>
            <tr><td style="padding:10px 14px;color:#64748b;">${etiquetaQuien}</td><td style="padding:10px 14px;font-weight:700;color:#1e293b;">${quien}</td></tr>
          </table>
          ${fichaUrl ? `<p style="margin:18px 0 0;font-size:14px;"><a href="${escapeHtml(fichaUrl)}" style="color:#0ea5e9;font-weight:600;">Abrir ficha del contacto</a></p>` : ''}
        </td></tr>
        <tr><td style="padding:14px 28px;background:#f8fafc;text-align:center;">
          <p style="margin:0;font-size:12px;color:#94a3b8;">Correo automático · CRM Gemavip</p>
        </td></tr>
      </table>
    </td></tr>
  </table>
</body></html>`;
  return sendPedidoEmail(to, { subject, text, html });
}
...
module.exports = {
  ...
  sendPedidoEmail,
  sendHoldedSyncPendingDigestEmail,
  sendHoldedSyncApprovalRequestEmail,
  sendHoldedSyncAppliedNotifyEmail,
  computeSigSyncCliente,
  sendTransferExcelEmail,
  sendTestEmail,
  sendAsignacionResultadoEmail,
  sendClienteModificadoEmail,
  APP_BASE_URL
};
