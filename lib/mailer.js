...
/**
 * Email de notificación genérica (nuevas notificaciones del CRM).
 * @param {string} to - Email destino
 * @param {{ title: string, body: string, url?: string }} data
 * @returns {Promise<{ sent: boolean, error?: string }>}
 */
async function sendNotificationEmail(to, data = {}) {
  const title = String(data.title || 'CRM Gemavip').trim();
  const body = String(data.body || '').trim();
  const url = String(data.url || '').trim();
  const link = url ? `\n\nVer: ${APP_BASE_URL}${url.startsWith('/') ? url : '/' + url}` : '';
  ...
  if (graphStatus && graphStatus.configured) {
    const r = await sendMailViaGraph(to, { subject, text });
    if (r.sent) return { sent: true };
    console.warn('[MAILER] Graph falló (notificación), intentando SMTP:', r.error);
  }
  ...
  try {
    ...
    console.warn('[MAILER] Error enviando email notificación:', err?.message);
    return { sent: false, error: err?.message };
  }
}
