...
/**
 * Alcance de tags: sin selección (y sin `HOLDED_SYNC_REQUIRE_TAGS`) se consideran
 * todos los client/lead que cumplan CIF + provincia ES. Opcional: `SYNC_HOLDED_DEFAULT_TAGS`
 * (coma-separadas) se fusionan con las elegidas en la UI.
 *
 * @param {string[]|undefined} selectedTags
 * @returns {{ mode: 'all'|'filter'|'require_empty', effectiveSet: Set<string>, effectiveTagsDisplay: string[] }}
 */
function resolveEffectiveTagSelection(selectedTags) {
  ...
  const requireTags = /^1|true|yes$/i.test(String(process.env.HOLDED_SYNC_REQUIRE_TAGS || '').trim());

  if (merged.size === 0) {
    if (requireTags) {
      return { mode: 'require_empty', effectiveSet: new Set(), effectiveTagsDisplay: [] };
    }
    return { mode: 'all', effectiveSet: new Set(), effectiveTagsDisplay: [] };
  }
  return {
    mode: 'filter',
    effectiveSet: merged,
...
/** @param {string[]} contactTags @param {{ mode: string, effectiveSet: Set<string> }} scope */
function contactMatchesEffectiveScope(contactTags, scope) {
  if (!scope || scope.mode === 'all') return true;
  if (scope.mode === 'require_empty') return false;
...
async function exportCrmClienteToHolded(db, cliId) {
  ...
  try {
    ...
    if (rowAfter) {
      ...
      await db.query('UPDATE clientes SET cli_holded_sync_hash = ? WHERE cli_id = ?', [h, cliId]);
    }
  } catch (eh) {
...
async function buildHoldedPreviewRowsFromList(db, list, selectedTagsRaw, opt = {}) {
  ...
  for (const c of list) {
    ...
    if (scope.mode === 'require_empty') {
      omitSinTag++;
      motivo =
        'Selecciona al menos una tag en la lista superior (o define SYNC_HOLDED_DEFAULT_TAGS; o quita HOLDED_SYNC_REQUIRE_TAGS)';
    } else if (scope.mode === 'filter' && !coincide) {
      omitSinTag++;
      motivo = 'Sin ninguna de las tags del filtro (UI + SYNC_HOLDED_DEFAULT_TAGS)';
    } else if (!cifOk) {
      omitSinCif++;
...
async function importHoldedSinCifComoLeads(db, opts = {}) {
  const selectedTagsArr = Array.isArray(opts.selectedTags)
    ? opts.selectedTags
    : parseSelectedTagsInput(opts.selectedTags || '');
  const scope = resolveEffectiveTagSelection(selectedTagsArr);
  const apiKey = (process.env.HOLDED_API_KEY || '').trim();
  if (!apiKey) {
    return { ok: false, error: 'Falta HOLDED_API_KEY en variables de entorno', inserted: 0, skipped: 0, errors: 0 };
  }
  if (scope.mode === 'require_empty' || scope.mode === 'all') {
    return {
      ok: false,
      error:
        'Para importar leads sin CIF, acota el alcance: selecciona al menos una tag en la lista o define SYNC_HOLDED_DEFAULT_TAGS (no uses el modo «todos los contactos» para este flujo).',
      inserted: 0,
      skipped: 0,
      errors: 0
    };
