...
const {
  ...
  getCrmColumnLabelEs
} = require('../holded-contact-field-map-ui');
const {
  loadGlobalHoldedFieldMap,
  getGlobalHoldedFieldMapMeta,
  invertPathToCrmMap,
  CRM_COLUMN_DENY
} = require('../holded-field-map-store');
const { getByPath, setByPath } = require('../holded-path-value');
const {
...
} = require('../holded-contact-normalize');
...
function crmRowToComparablePayload(row) {
  ...
  p.cli_idiom_id = g(['cli_idiom_id', 'Id_Idioma']);
  p.cli_mon_id = g(['cli_mon_id', 'Id_Moneda']);
  p.cli_dto = g(['cli_dto', 'Dto']);
  p.cli_tarifa_legacy = g(['cli_tarifa_legacy', 'Tarifa']);
  p.cli_Modelo_347 = g(['cli_Modelo_347', 'Modelo_347']);
  return p;
}
...
function normComparableScalar(v) {
  ...
  if (typeof v === 'object') {
    const s = holdedAccountLikeToString(v);
    return s ? s.trim() : '';
  }
...
/**
 * Holded a veces devuelve `salesAccount` / `clientRecord` (y análogos) como objeto, no como código en string.
 * @param {unknown} raw
 * @returns {string}
 */
function holdedAccountLikeToString(raw) {
  if (raw == null) return '';
  ...
  if (typeof raw === 'object') {
    if (Array.isArray(raw)) {
      return raw.map(holdedAccountLikeToString).filter(Boolean).join(', ');
    }
    const o = /** @type {Record<string, unknown>} */ (raw);
    // Preferir código / número de cuenta contable frente al nombre legible (p. ej. Holded devuelve ambos).
    const pick =
      o.number ??
      o.accountNumber ??
      o.numAccount ??
...
function buildClientePayloadFromHoldedContact(contact, ctx = {}) {
  ...
  const mergedFiscal = mergeHoldedFiscalCodeForStorage(contact);
  const cif = mergedFiscal && mergedFiscal.length ? mergedFiscal : '';
  ...
  const web = pickWebsiteFromHoldedContact(contact);
  if (web) payload.Web = web;
  ...
  const cvStr = holdedAccountLikeToString(cv);
  if (cvStr) payload.cli_cuenta_ventas = cvStr;
  const cc = contact?.supplierRecord ?? contact?.expensesAccount;
  const ccStr = holdedAccountLikeToString(cc);
  if (ccStr) payload.cli_cuenta_compras = ccStr;
  ...
  const tipoIsp = tipoContactoFromHoldedIsperson(contact);
  if (tipoIsp) {
    payload.TipoContacto = tipoIsp;
  } else {
  ...
  const defHolded = contact?.defaults;
  if (defHolded && typeof defHolded === 'object') {
    if (defHolded.discount != null && defHolded.discount !== '') {
      const d = Number(defHolded.discount);
      if (Number.isFinite(d)) payload.cli_dto = d;
    }
    const m347 = modelo347FromHoldedDefaults(defHolded);
    if (m347 !== null) payload.cli_Modelo_347 = m347;
  }

  if (ctx.idiomId != null && Number.isFinite(Number(ctx.idiomId))) {
    payload.cli_idiom_id = Number(ctx.idiomId);
  }
  if (ctx.idiomaCodigo != null && String(ctx.idiomaCodigo).trim() !== '') {
    payload.cli_Idioma = String(ctx.idiomaCodigo).trim();
  }
  if (ctx.monId != null && Number.isFinite(Number(ctx.monId))) {
    payload.cli_mon_id = Number(ctx.monId);
  }
  if (ctx.monCodigo != null && String(ctx.monCodigo).trim() !== '') {
    payload.cli_Moneda = String(ctx.monCodigo).trim();
  }
  if (ctx.formpId != null && Number.isFinite(Number(ctx.formpId))) {
    payload.cli_formp_id = Number(ctx.formpId);
  }
  if (ctx.tarifaLegacy != null && Number.isFinite(Number(ctx.tarifaLegacy))) {
    payload.cli_tarifa_legacy = Number(ctx.tarifaLegacy);
  }
...
function buildHoldedPutBodyFromCrmRow(crm, provNombre) {
  ...
  let cif = '';
  if (cifRaw) {
    const s = String(cifRaw).trim();
    cif = normalizeDniCifForStorage(s) || s;
  }
  ...
  if (cif && cif.length >= 8 && String(cif).toUpperCase() !== 'PENDIENTE') {
    body.code = cif;
    body.vatnumber = cif;
  }
  ...
  if (tipoCont) {
    ...
    if (tl === 'persona') body.isperson = 1;
    else if (tl === 'empresa') body.isperson = 0;
  }
  ...
  /** @type {Record<string, unknown>} */
  const defaultsPut = {};
  const langPut = g(['cli_Idioma', 'Idioma']);
  if (langPut) defaultsPut.language = String(langPut).trim().toLowerCase().slice(0, 8);
  const monPut = g(['cli_Moneda', 'Moneda']);
  if (monPut) defaultsPut.currency = String(monPut).trim().toLowerCase().slice(0, 8);
  const dtoPut = crm.cli_dto ?? crm.Dto;
  if (dtoPut != null && String(dtoPut).trim() !== '') {
    const dn = Number(dtoPut);
    if (Number.isFinite(dn)) defaultsPut.discount = dn;
  }
  const m347Put = crm.cli_Modelo_347 ?? crm.Modelo_347;
  if (m347Put === 0 || m347Put === 1 || m347Put === '0' || m347Put === '1') {
    defaultsPut.accumulateInForm347 = Number(m347Put) === 1 ? 'yes' : 'no';
  }
  if (Object.keys(defaultsPut).length) body.defaults = defaultsPut;
 ...
 * @returns {unknown}
 */
function coerceHoldedLeafForCrmColumn(col, raw) {
  ...
  if (fkLike || lc === 'cli_dto') {
    const n = Number(raw);
    return Number.isFinite(n) ? n : null;
  }
  if (typeof raw === 'object' && raw !== null && !Array.isArray(raw)) {
    const s = holdedAccountLikeToString(raw);
    return s || null;
  }
  if (Array.isArray(raw)) {
    return raw.map((x) => (x && typeof x === 'object' ? String(JSON.stringify(x)) : String(x))).join(', ');
  }
  return raw === null ? null : String(raw);
}

/**
 * Tras el payload base por contacto Holded, aplica mapeo manual global (ruta → columna CRM).
 * @param {object} contact
 * @param {Record<string, unknown>} payload
 * @param {Record<string, string>} savedMap
 */
function applySavedHoldedImportMapping(contact, payload, savedMap) {
  if (!contact || !payload || !savedMap) return;
  for (const [path, col] of Object.entries(savedMap)) {
    if (!path || !col) continue;
    if (CRM_COLUMN_DENY.has(col)) continue;
    const v = getByPath(contact, path);
    if (v === undefined) continue;
    const coerced = coerceHoldedLeafForCrmColumn(col, v);
    if (coerced !== undefined) payload[col] = coerced;
  }
}

/**
 * Mezcla valores CRM en el cuerpo PUT según mapeo guardado (columna → ruta Holded).
 * @param {Record<string, unknown>} body
 * @param {Record<string, unknown>} crm
 * @param {Record<string, string>} savedMap path → col
 */
function applySavedHoldedExportMapping(body, crm, savedMap) {
  if (!body || !crm || !savedMap) return;
  for (const [path, col] of Object.entries(savedMap)) {
    if (!path || !col) continue;
    if (CRM_COLUMN_DENY.has(col)) continue;
    let v = crm[col];
    ...
    setByPath(body, path, v);
  }
}

/**
 * @param {Record<string, unknown>} crm
 * @param {Record<string, string>} savedFieldMap
 */
function buildCrmPickerRowsForComparar(crm, savedFieldMap) {
  const rev = invertPathToCrmMap(savedFieldMap || {});
  return CRM_CLIENTE_COLUMN_OPTIONS.map((col) => ({
    col,
    colLabelEs: getCrmColumnLabelEs(col),
    valueDisplay: formatSnapshotCellValue(crm[col]),
    selectedHoldedPath: rev[col] || ''
  }));
}
...
async function exportCrmClienteToHolded(db, cliId) {
  ...
  try {
    const savedMap = await loadGlobalHoldedFieldMap(db);
    applySavedHoldedExportMapping(body, crm, savedMap);
  } catch (_) {
    /* */
  }
  try {
  ...
  try {
    ...
    const ctx = await buildHoldedSyncContext(db, fresh);
...
async function importCrmClienteFromHolded(db, cliId) {
  ...
  const ctx = await buildHoldedSyncContext(db, full);

  const payload = buildClientePayloadFromHoldedContact(full, ctx);
  applyHoldedTipoLeadRulesToPayload(payload, full, { tipcLeadId, existingCrmRow: crm });
  try {
    const savedMap = await loadGlobalHoldedFieldMap(db);
    applySavedHoldedImportMapping(full, payload, savedMap);
  } catch (_) {
    /* */
  }
...
function formatCrmComparableDisplay(key, pC, crm) {
  ...
  if (key === 'cli_cuenta_ventas' || key === 'cli_cuenta_compras') {
    const raw = pC[key];
  ...
  if (key === 'cli_idiom_id') {
    const id = pC.cli_idiom_id != null && pC.cli_idiom_id !== '' ? String(pC.cli_idiom_id) : '';
    const name = crm?.IdiomaNombre != null ? String(crm.IdiomaNombre).trim() : '';
    if (!name && !id) return '—';
    return [name, id ? `id: ${id}` : ''].filter(Boolean).join(' · ');
  }
  if (key === 'cli_mon_id') {
    const id = pC.cli_mon_id != null && pC.cli_mon_id !== '' ? String(pC.cli_mon_id) : '';
    const name = crm?.MonedaNombre != null ? String(crm.MonedaNombre).trim() : '';
    if (!name && !id) return '—';
    return [name, id ? `id: ${id}` : ''].filter(Boolean).join(' · ');
  }
  if (key === 'cli_tarifa_legacy') {
    const id = pC.cli_tarifa_legacy != null && pC.cli_tarifa_legacy !== '' ? String(pC.cli_tarifa_legacy) : '';
    return id ? `tarifa id: ${id}` : '—';
  }
...
function formatHoldedComparableDisplay(key, pH, contact, ctx) {
  ...
  if (key === 'cli_cuenta_ventas') {
    const raw = contact?.clientRecord ?? contact?.salesAccount;
    if (raw !== undefined && raw !== null && raw !== '') {
      const s = holdedAccountLikeToString(raw);
      return s === '' ? '—' : s;
    }
  }
  if (key === 'cli_cuenta_compras') {
    const raw = contact?.supplierRecord ?? contact?.expensesAccount;
    if (raw !== undefined && raw !== null && raw !== '') {
      const s = holdedAccountLikeToString(raw);
      return s === '' ? '—' : s;
    }
  ...
  if (key === 'cli_idiom_id') {
    const lang = contact?.defaults?.language != null ? String(contact.defaults.language).trim() : '';
    const id = ctx?.idiomId != null && ctx.idiomId !== '' ? String(ctx.idiomId) : '';
    const hid = pH?.cli_idiom_id != null ? String(pH.cli_idiom_id) : '';
    const parts = [lang ? `Holded: ${lang}` : '', id ? `idiom_id: ${id}` : hid ? `idiom_id: ${hid}` : ''].filter(Boolean);
    return parts.length ? parts.join(' · ') : '—';
  }
  if (key === 'cli_mon_id') {
    const cur = contact?.defaults?.currency != null ? String(contact.defaults.currency).trim().toUpperCase() : '';
    const id = ctx?.monId != null && ctx.monId !== '' ? String(ctx.monId) : '';
    const hid = pH?.cli_mon_id != null ? String(pH.cli_mon_id) : '';
    const parts = [cur ? `Holded: ${cur}` : '', id ? `mon_id: ${id}` : hid ? `mon_id: ${hid}` : ''].filter(Boolean);
    return parts.length ? parts.join(' · ') : '—';
  }
  if (key === 'cli_dto') {
    const d = contact?.defaults?.discount;
    if (d != null && d !== '') return String(d);
  }
  if (key === 'cli_tarifa_legacy') {
    const rn = contact?.rate?.name != null ? String(contact.rate.name).trim() : '';
    const tid = ctx?.tarifaLegacy != null ? String(ctx.tarifaLegacy) : '';
    const hid = pH?.cli_tarifa_legacy != null ? String(pH.cli_tarifa_legacy) : '';
    const parts = [rn ? `rate: ${rn}` : '', tid || hid ? `tarifa id: ${tid || hid}` : ''].filter(Boolean);
    return parts.length ? parts.join(' · ') : '—';
  }
  if (key === 'cli_Modelo_347') {
    const v = contact?.defaults?.accumulateInForm347;
 ...
 *   crmAllColumnas: Array<{ key: string, value: string }>,
 *   holdedJsonPretty: string|null,
 *   holdedFieldRows: Array<object>,
 *   crmColumnOptions: string[]
 * }>}
 */
function buildCrmAllColumnas(crm) {
  if (!crm || typeof crm !== 'object') return [];
  return Object.keys(crm)
    .sort((a, b) => a.localeCompare(b, 'es'))
    .map((k) => ({
      key: k,
      value: formatSnapshotCellValue(crm[k]),
      labelEs: getCrmColumnLabelEs(k)
    }));
}
 ...
 * @returns {string|null}
 */
function safeHoldedJsonPretty(contact) {
  if (!contact || typeof contact !== 'object') return null;
  try {
...
async function buildClienteHoldedComparisonDetail(db, cliId) {
  ...
  if (!Number.isFinite(numId) || numId <= 0) {
    return {
      ...
      comparableRows: [],
      crmAllColumnas: [],
      holdedJsonPretty: null,
      holdedFieldRows: [],
      crmColumnOptions: CRM_CLIENTE_COLUMN_OPTIONS,
      savedFieldMap: {},
      fieldMapMeta: { updatedAt: null, updatedBy: null },
      crmPickerRows: [],
      holdedPathOptions: [],
      holdedPathLabels: {},
      crmColumnLabelsEs: {}
  ...
  const crmColumnLabelsEs = Object.fromEntries(
    CRM_CLIENTE_COLUMN_OPTIONS.map((c) => [c, getCrmColumnLabelEs(c)])
  );
  ...
  const crmAllColumnas = buildCrmAllColumnas(crm);

  /** @type {Record<string, string>} */
  let savedFieldMap = {};
  /** @type {{ updatedAt: string | null, updatedBy: string | null }} */
  let fieldMapMeta = { updatedAt: null, updatedBy: null };
  try {
    savedFieldMap = await loadGlobalHoldedFieldMap(db);
    fieldMapMeta = await getGlobalHoldedFieldMapMeta(db);
  } catch (_) {
    /* */
  }
  const crmPickerBase = buildCrmPickerRowsForComparar(crm, savedFieldMap);
  ...
  const flatRows = buildHoldedFieldMapRows(full);
  const holdedFieldRowsMerged = mergeFieldMapIntoRows(flatRows, savedFieldMap);
  const holdedPathOptions = flatRows.map((r) => r.path);
  /** @type {Record<string, string>} */
  const holdedPathLabels = {};
  for (const r of holdedFieldRowsMerged) {
    if (r.path) holdedPathLabels[r.path] = r.pathLabelEs || r.path;
  }
  ...
  return {
    ...
    comparableRows,
    crmAllColumnas,
    holdedJsonPretty: safeHoldedJsonPretty(full),
    holdedFieldRows: holdedFieldRowsMerged,
    crmColumnOptions: CRM_CLIENTE_COLUMN_OPTIONS,
    savedFieldMap,
    fieldMapMeta,
    crmPickerRows: buildCrmPickerRowsForComparar(crm, savedFieldMap),
    holdedPathOptions,
    holdedPathLabels,
    crmColumnLabelsEs
  };
...
async function evaluateCliHoldedSyncPendienteAfterCrmSave(db, cliId, opts = {}) {
  ...
  try {
    ...
    const ctx = await buildHoldedSyncContext(db, contact);
...
/** CIF / NIF en Holded: `code` o `vatnumber` (fusionados en CRM). */
function hasCifHolded(contact) {
  return mergeHoldedFiscalCodeRaw(contact) !== '';
}
...
/**
 * Provincia/país + catálogo (idioma, moneda, forma pago, tarifa) para hash e import.
 * @param {import('../config/mysql-crm')} db
 * @param {object} contact
 */
async function buildHoldedSyncContext(db, contact) {
  const paisId = await getPaisIdEspana(db);
  const provStr = getProvinceFromContact(contact);
  const provRow = provStr ? await findProvinciaEspana(db, provStr) : null;
  const cat = await resolveHoldedClienteCatalogMaps(db, contact);
  return {
...
async function enrichHoldedRowsWithSyncState(db, rows, list, paisIdEspana) {
  ...
  for (const row of candidatos) {
    ...
    const ctx = await buildHoldedSyncContext(db, c);
...
function buildContactRow(contact, { provRow, tags, paisId }) {
  ...
  const cifMerged = mergeHoldedFiscalCodeForStorage(contact);
  const cif = cifMerged && cifMerged.length ? cifMerged : null;
  const cpRaw = bill?.postalCode ?? ship0?.postalCode;
...
async function importHoldedClientesEs(db, opts = {}) {
  ...
  for (const row of toImport) {
    ...
    try {
      ...
      const ctxImp = await buildHoldedSyncContext(db, full);
      const payload = buildClientePayloadFromHoldedContact(full, ctxImp);
      ...
      if (cliIdAfter != null) {
        try {
          ...
          if (rowAfter) {
            const ctxHash = await buildHoldedSyncContext(db, full);
...
async function importHoldedSinCifComoLeads(db, opts = {}) {
  ...
  for (const row of targets) {
    ...
    try {
      ...
      const ctxLead = await buildHoldedSyncContext(db, full);
      const payload = buildClientePayloadFromHoldedContact(full, ctxLead);
      if (payload.cli_com_id === 1) {
        payload.cli_com_id = defaultComId;
      }
      payload.cli_estcli_id = leadEstId;
