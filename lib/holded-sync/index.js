...
/** Texto columna Tags en CPanel: solo `crm` si el contacto la tiene; no lista otras etiquetas Holded. */
function tagsDisplayCrmOnly(contactTags) {
  const arr = Array.isArray(contactTags) ? contactTags : [];
  const has = arr.some((t) => String(t).toLowerCase().trim() === 'crm');
  return has ? 'crm' : '—';
}
...
/**
 * Filas vista previa con estado omitido: marca si ya existe cliente CRM con mismo ID Holded (no mostrar en «errores»).
 * @param {import('../config/mysql-crm')} db
 * @param {object[]} rows
 */
async function enrichOmitidosRowsCrmYaImportados(db, rows) {
  const omitidos = (rows || []).filter((r) => r.estadoBase === 'omitido' && r.holdedId);
  ...
  try {
    ...
    console.warn('[holded-sync] enrichOmitidosRowsCrmYaImportados:', e?.message || e);
    return;
  }
  const inCrm = new Set();
  ...
  for (const row of rows) {
    if (row.estadoBase !== 'omitido' || !row.holdedId) continue;
    const hid = String(row.holdedId).trim();
    row.crmYaExisteEnCrm = inCrm.has(hid);
  }
}
...
/**
 * UI CPanel: solo la etiqueta `crm` (conteo de contactos client/lead que la tienen).
 * No se listan otras tags de Holded en el filtro.
 * @param {object[]} list
 */
function computeCrmTagMetaForUi(list) {
  let count = 0;
  for (const c of list || []) {
    const tags = normalizeHoldedTags(c);
    if (tags.some((t) => String(t).toLowerCase().trim() === 'crm')) count++;
  }
  return [{ key: 'crm', display: 'crm', count }];
}
...
async function previewHoldedClientesEs(db, opts = {}) {
  ...
  try {
    ...
    const allTags = computeCrmTagMetaForUi(list);
    const paisId = await getPaisIdEspana(db);
