...
async function importCrmClienteFromHolded(db, cliId) {
  ...
  try {
    ...
    if (rowAfter) {
      const h = hashFromCrmRow(rowAfter);
      await db.query('UPDATE clientes SET cli_holded_sync_hash = ?, cli_holded_sync_pendiente = 0 WHERE cli_id = ?', [
        h,
        cliId
      ]);
    }
...
async function evaluateCliHoldedSyncPendienteAfterCrmSave(db, cliId, opts = {}) {
  ...
  if (fromView) {
    ...
    if (last != null && Date.now() - last < HOLDED_VIEW_EVAL_COOLDOWN_MS) return;
  }
  const apiKey = (process.env.HOLDED_API_KEY || '').trim();
  if (!apiKey) return;
  ...
  try {
    ...
    if (pend) {
      await maybeQueueHoldedSyncApprovalNotif(db, id, { contact, crm, ctx, pteExportar, pteImportar, hid });
...
async function maybeQueueHoldedSyncApprovalNotif(db, cliId, p) {
  ...
  try {
    ...
    const to = String(process.env.HOLDED_SYNC_NOTIFY_EMAIL || 'p.lara@gmavip.com').trim();
    const nombre = String(p.crm.cli_nombre_razon_social ?? p.crm.Nombre_Razon_Social ?? '').trim() || `Cliente #${id}`;
    await sendHoldedSyncApprovalRequestEmail(to, {
      notifId,
      cliId: id,
      clienteNombre: nombre,
      diffCampos: notasObj.diffCampos,
      sugerencia
    }).catch((e) => console.warn('[holded-sync] sendHoldedSyncApprovalRequestEmail:', e?.message || e));
  } catch (e) {
