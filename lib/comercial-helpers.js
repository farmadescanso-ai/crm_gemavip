...
async function loadComercialesTableMeta() {
  try {
    ...
    const hasFechaBaja = await db.comercialesHasFechaBajaColumn().catch(() => false);
    return {
    ...
    return { hasMeetEmail: false, hasTeamsEmail: false, hasPlataforma: true, hasFijoMensual: true, hasActivo: false, hasFechaBaja: false };
  }
}
...
/** Fila de comercial (o objeto con com_activo/Activo). Si no hay columna, se considera activo salvo fecha de baja. */
function comercialRowIsActive(row) {
  ...
  const fb = row.com_fecha_baja ?? row.FechaBaja;
  if (fb != null && String(fb).trim() !== '') return false;
  return true;
}
