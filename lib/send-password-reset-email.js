...
/**
 * Recuperación de contraseña del portal del cliente.
 */
async function sendPortalPasswordResetEmail(toEmail, resetLink, nombreCliente) {
  const transport = getTransporter();
  if (!transport) return false;
  const from = process.env.MAIL_FROM || process.env.SMTP_USER || 'noreply@crm-gemavip.local';
  const subject = 'Restablecer contraseña · Portal cliente Gemavip';
  const greet = nombreCliente ? `Hola ${nombreCliente},` : 'Hola,';
  const text = [
    greet,
    '',
    'Has solicitado restablecer tu contraseña del portal de cliente.',
  ...
  const html = [
    ...
    '<p>Has solicitado restablecer tu contraseña del <strong>portal de cliente</strong>.</p>',
    `<p><a href="${escapeHtml(resetLink)}" style="background:#0d9488;color:#fff;padding:10px 16px;text-decoration:none;border-radius:8px;">Restablecer contraseña</a></p>`,
  ...
  try {
    ...
    console.error('❌ Error email recuperación portal:', err?.message);
    return false;
  }
}

/**
 * Invitación al portal (acceso creado por comercial).
 */
async function sendPortalInviteEmail(toEmail, portalLoginUrl, nombreCliente) {
  const transport = getTransporter();
  if (!transport) return false;
  const from = process.env.MAIL_FROM || process.env.SMTP_USER || 'noreply@crm-gemavip.local';
  const subject = 'Tu acceso al portal de cliente · Gemavip';
  const greet = nombreCliente ? `Hola ${nombreCliente},` : 'Hola,';
  const text = [
    greet,
    '',
    'Te han activado el acceso al portal de cliente (aplicación independiente del CRM comercial).',
    'Tu usuario es este email. La contraseña te la comunicará Gemavip por el canal acordado.',
    'Entra aquí:',
    portalLoginUrl,
    '',
    '— Gemavip'
  ...
  const html = [
    `<p>${escapeHtml(greet)}</p>`,
    '<p>Te han activado el acceso al <strong>portal de cliente</strong> (canal externo, distinto del acceso de comerciales).</p>',
    '<p>Tu usuario es este email; la contraseña te la facilitará Gemavip por el canal acordado.</p>',
    `<p><a href="${escapeHtml(portalLoginUrl)}" style="background:#0d9488;color:#fff;padding:10px 16px;text-decoration:none;border-radius:8px;">Ir al portal</a></p>`
  ].join('');
  try {
    await transport.sendMail({ from, to: toEmail, subject, text, html });
    return true;
  } catch (err) {
    console.error('❌ Error email invitación portal:', err?.message);
    return false;
  }
}
