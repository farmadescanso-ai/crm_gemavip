...
async function sendPortalInviteEmail(toEmail, portalLoginUrl, nombreCliente) {
  ...
  const greet = nombreCliente ? `Hola ${nombreCliente},` : 'Hola,';
  const text = [greet, '', 'Te han activado el acceso al portal de cliente.', 'Entra aquí:', portalLoginUrl, '', '— Gemavip'].join('\n');
  const html = [
    `<p>${escapeHtml(greet)}</p>`,
    '<p>Te han activado el acceso al <strong>portal de cliente</strong>.</p>',
